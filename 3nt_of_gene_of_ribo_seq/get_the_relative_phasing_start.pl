...
for(my $number=0;$number<=50;$number++){
	my $aa = $number - 2;
	my $bb = $number - 1;
	my $cc = $number + 1;
	my $dd = $number + 2;
	my $average;
	if($number == 49){
		$average = ($hash_read_number{$aa}+$hash_read_number{$bb}+$hash_read_number{$cc})/3;
	}
	elsif($number == 50){
		$average = ($hash_read_number{$aa}+$hash_read_number{$bb})/2;
	}
	else{
